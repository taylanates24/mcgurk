...
keyboard is the one part that is faked — a test cannot press a key — and faking
it is what makes the rest checkable: that the fixation, the presentation, the
response and the database writes happen in that order, that both reaction times
are recorded, that a timeout leaves no response row, and that the block closes
(and therefore commits) exactly once.
...
class ScriptedKeyboard:
    ...
    def getKeys(  # noqa: N802 - PsychoPy's spelling; this stands in for it
        ...
        self._trial += 1
        self._polls = 0
        return [_Press(name=key, rt=self.rt_s)]
